...
module.exports = {
    instance: async (dbName = 'huobi', force = false) => {
        ...
        if (!dbInstance[dbName] || force) {
            console.info(`##### [${moment().format()}] Start connection [${dbName}] #####`);
            try {
                ...
                console.info(`##### [${moment().format()}] Connection Database [${dbName}] time to (${(+new Date() - st) / 1000})sec #####`);
            } catch (e) {
                console.error(`##### [${moment().format()}] [Mongodb] connection [${dbName}] error #####`, e);
